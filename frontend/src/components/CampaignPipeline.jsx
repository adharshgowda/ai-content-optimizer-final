...
const STEPS = [
  ...
  { id: 'abtest',     icon: '⚔️', label: 'A/B Prediction',      desc: 'Picking the highest-performing variant' },
...
export default function CampaignPipeline() {
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      // ── STEP 4: A/B Test top 2 ──────────────────────────────────────
      setStep('abtest', STATUS.running);
      const varA = sentimentResults[0]?.text || '';
      const varB = sentimentResults[1]?.text || sentimentResults[0]?.text || '';
      let winner = varA; let abResult = null;
      try {
        const abRes = await fetch('/api/ab-test', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ variantA: varA, variantB: varB })
        });
        abResult = (await abRes.json()).result;
        winner = abResult?.winner === 'B' ? varB : varA;
      } catch { winner = varA; }
      setStep('abtest', STATUS.done, { winner, explanation: abResult?.explanation, scoreA: abResult?.scoreA, scoreB: abResult?.scoreB });
      await delay(400);

      // ── STEP 5: Send to Slack ────────────────────────────────────────
      setStep('slack', STATUS.running);
      const slackMsg = `🏆 *Campaign Pipeline Result*\n📊 Platform: ${platform} | Tone: ${tone}\n🔗 Source: ${url}\n\n✅ *Winning Post:*\n${winner}\n\n_Sentiment Score: ${sentimentResults[0]?.score?.toFixed(2)} | A/B Winner: Variant ${abResult?.winner || 'A'}_`;
      try {
      ...
      try {
        await fetch('/api/metrics/record-demo', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ winner: abResult?.winner || 'A', score: abResult?.scoreA || 0.8 })
        });
      } catch {}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '0.82rem', color: '#475569' }}>
                      {isRunning ? step.desc :
                       ...
                       isDone && step.id === 'abtest' ? `✓ Scores — A: ${data?.scoreA?.toFixed(3)} · B: ${data?.scoreB?.toFixed(3)}` :
            ...
            <div style={{ ...card, background: 'rgba(34,197,94,0.04)', border: '1px solid rgba(34,197,94,0.2)', animation: 'fadeUp 0.5s ease forwards', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '1.5rem' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.82rem', color: '#64748b' }}>Highest A/B score · Best sentiment · Ready to publish</div>
                </div>
