  ...
  const analyzeAll = async () => {
    ...
    try {
      ...
      for (const v of initialVariants) {
        const res = await fetch('/api/sentiment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: v.text }) });
        const data = await res.json();
        arr.push({ text: v.text, sentiment: data.sentiment });
      }
      setResults(arr);
