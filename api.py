...
def run_ab_test(req: ABTestRequest):
    if not ABCoach:
        raise HTTPException(status_code=500, detail="A/B Coach not found")
    try:
        coach = ABCoach()
        out = coach.simulate_ab(req.variantA, req.variantB)
        
        # Optional auto slack alert on A/B test completion
        if SlackNotifier:
            try:
